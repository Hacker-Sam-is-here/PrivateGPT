...
from .exceptions import Private_GPTE
...
def json_dumps(obj: Any) -> str:
    ...
    Raises:
        Private_GPTE: If serialization fails
    """
    try:
        return json.dumps(obj, ensure_ascii=False, indent=2)
    except Exception as ex:
        raise Private_GPTE(f"{type(ex).__name__}: {ex}") from ex
...
def json_loads(obj: Union[str, bytes]) -> Any:
    ...
    Raises:
        Private_GPTE: If deserialization fails
    """
    try:
...
def _extract_vqd(html_bytes: bytes, keywords: str) -> str:
    ...
    for c1, c1_len, c2 in (
        ...
        try:
            ...
            pass
    raise Private_GPTE(f"_extract_vqd() {keywords=} Could not extract vqd.")
...
def _text_extract_json(html_bytes: bytes, keywords: str) -> List[Dict[str, str]]:
    ...
    try:
        ...
        raise Private_GPTE(f"_text_extract_json() {keywords=} {type(ex).__name__}: {ex}") from ex
    raise Private_GPTE(f"_text_extract_json() {keywords=} return None")
