"""
Private_GPT OpenAI-Compatible API Server
...
def run_api(
    ...
    print("Starting Private_GPT OpenAI API server...")
    if port is None:
        port = DEFAULT_PORT
    ...
    uvicorn_app_str = (
        "private_gpt.server.server:create_app_debug" if debug else "private_gpt.server.server:create_app"
    )
...
def main():
    """Main entry point for the private_gpt-server console script."""
    import argparse
