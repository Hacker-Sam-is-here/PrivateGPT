...
an available LLM provider from the private_gpt library that doesn't require
API keys or cookies.
"""
...
from private_gpt.exceptions import AllProvidersFailure


def load_providers() -> Tuple[Dict[str, Type[Provider]], set]:
    """
    Dynamically loads all Provider classes from the `private_gpt.Provider` package.

    This function iterates through the modules in the `private_gpt.Provider` package,
...
class AUTO(Provider):
    ...
    LLM provider from the private_gpt library.
