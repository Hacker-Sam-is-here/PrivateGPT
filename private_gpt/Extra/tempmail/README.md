...
Install or update Private_GPT to get access to the TempMail module:
...
```python
from private_gpt.Extra.tempmail import get_random_email
...
```python
from private_gpt.Extra.tempmail import get_provider
...
```bash
python -m private_gpt.Extra.tempmail.cli --help
```
...
python -m private_gpt.Extra.tempmail.cli --wait 15
```
...
### Asynchronous Utilities (`private_gpt.Extra.tempmail.async_utils`)
...
```python
import asyncio
from private_gpt.Extra.tempmail.async_utils import get_temp_email, wait_for_message
...
Adding support for a new temporary email service is straightforward thanks to the abstract provider pattern used throughout Private_GPT:

1.  **Create Provider Classes**: Implement two new classes:
    *   One inheriting from `private_gpt.Extra.tempmail.base.TempMailProvider` for synchronous operations.
    *   One inheriting from `private_gpt.Extra.tempmail.base.AsyncTempMailProvider` for asynchronous operations.
...
2.  **Register in Factory**: Update the `get_provider` function in `private_gpt/Extra/tempmail/base.py` to recognize a new `provider_name` string and return instances of your newly created classes.
3.  **Expose (Optional)**: If desired, update `private_gpt/Extra/tempmail/__init__.py` to directly import and expose your new provider classes for users who might want to instantiate them directly.
...
*   **Parser**: Uses the `Scout` HTML/XML parser from the core `private_gpt` library instead of external libraries like `BeautifulSoup` for consistency and potentially better performance within the Private_GPT ecosystem.
...
*   **Minimal Dependencies**: Avoids adding external logging or unnecessary dependencies to ensure smooth integration into projects using Private_GPT.
...
Please refer to the main Private_GPT project's contributing guidelines if you plan to submit a pull request.
...
<div align="center">
  ...
  <p>📧 TempMail - Part of the Private_GPT Toolkit</p>
  <a href="https://github.com/pyscout/Private_GPT">Back to Main Private_GPT Project</a>
</div>