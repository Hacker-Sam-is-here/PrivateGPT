...
PYPI_URL = "https://pypi.org/pypi/private_gpt/json"
YOUTUBE_URL = "https://youtube.com/@OEvortex"
GITHUB_URL = "https://github.com/OEvortex/Private_GPT"
CACHE_FILE = Path(tempfile.gettempdir()) / "private_gpt_update_check.cache"

session = Session()


def get_installed_version() -> str:
    """Get the currently installed version of private_gpt."""
    try:
    ...
    try:
        import private_gpt.version

        return private_gpt.version.__version__
    except (ImportError, AttributeError):
        pass

    try:
        return importlib.metadata.version("private_gpt")
...
def format_update_message(current: str, new: str, utype: str) -> str:
    ...
    cmd = "pip install -U private_gpt"
    if utype == "Pre-release" or version.parse(new).is_prerelease:
        cmd = "pip install -U --pre private_gpt"
    ...
    if HAS_RICH:
        ...
        content = Text.assemble(
            ...
            ("version of Private_GPT is available!\n\n", "white"),
            ("Current:     ", "white"),
            (f"{current}", "bold red"),
            ("\n", ""),
            ("Latest:      ", "white"),
        ...
        return (
            f"\n\033[1;36m[ Private_GPT Update ]\033[0m\n"
...
def format_dev_message(current: str, latest: str) -> str:
    ...
    if HAS_RICH:
        ...
        panel = Panel(
            content,
            title="[bold blue]Private_GPT Dev Mode[/bold blue]",
            border_style="yellow",
            expand=False,
            padding=(1, 2),
        )
        ...
        return (
            f"\n\033[1;33m[ Private_GPT Info ]\033[0m\n"
...
def check_for_updates(force: bool = False) -> Optional[str]:
    """
    Check if a newer version of Private_GPT is available.
