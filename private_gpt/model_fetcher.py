...
CACHE_DIR = Path(tempfile.gettempdir()) / "private_gpt"


class ModelFetcherCache:
    """Thread-safe file-based cache for model lists with TTL support.

    Stores cached model data in the system temp directory under `private_gpt/model_cache.json`
    with per-provider expiration times. Supports disabling via `private_gpt_NO_MODEL_CACHE`
    env var.
    ...
    def __init__(self, cache_ttl: Optional[int] = None, debug: bool = False) -> None:
        ...
        Args:
            cache_ttl: TTL in seconds. If None, reads from private_gpt_MODEL_CACHE_TTL
        ...
        self.debug = debug
        self.cache_disabled = os.getenv("private_gpt_NO_MODEL_CACHE", "").lower() in (
            "1",
            "true",
            "yes",
        )
        ...
        if cache_ttl is not None:
            ...
            env_ttl = os.getenv("private_gpt_MODEL_CACHE_TTL")
