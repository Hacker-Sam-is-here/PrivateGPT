"""
Private_GPT Unified Client Interface

A unified client for private_gpt that provides a simple interface
...
from private_gpt.Provider.TTI.base import BaseImages, TTICompatibleProvider
from private_gpt.Provider.TTI.utils import ImageResponse
...
def load_openai_providers() -> Tuple[Dict[str, Type[OpenAICompatibleProvider]], Set[str]]:
    ...
    Scans the private_gpt.Provider.Openai_comp package and imports all subclasses of
    ...
    try:
        provider_package = importlib.import_module("private_gpt.Provider.Openai_comp")
        for _, module_name, _ in pkgutil.iter_modules(provider_package.__path__):
            if module_name.startswith(("base", "utils", "pydantic", "__")):
                continue
            try:
                module = importlib.import_module(f"private_gpt.Provider.Openai_comp.{module_name}")
...
def load_tti_providers() -> Tuple[Dict[str, Type[TTICompatibleProvider]], Set[str]]:
    ...
    Scans the private_gpt.Provider.TTI package and imports all subclasses of
    ...
    try:
        provider_package = importlib.import_module("private_gpt.Provider.TTI")
        for _, module_name, _ in pkgutil.iter_modules(provider_package.__path__):
            if module_name.startswith(("base", "utils", "__")):
                continue
            try:
                module = importlib.import_module(f"private_gpt.Provider.TTI.{module_name}")
                for attr_name in dir(module):
                    attr = getattr(module, attr_name)
                    if (
                        isinstance(attr, type)
...
def _get_models_safely(provider_cls: type, client: Optional["Client"] = None) -> List[str]:
    ...
    Examples:
        >>> from private_gpt.client import _get_models_safely, Client
        >>> client = Client()
        >>> from private_gpt.Provider.Openai_comp.some_provider import SomeProvider
...
class ClientCompletions(BaseCompletions):
    ...
    def _get_provider_instance(
        ...
        Examples:
            >>> from private_gpt.Provider.Openai_comp.gpt4free import GPT4Free
            >>> completions = client.chat.completions
...
class ClientImages(BaseImages):
    ...
    def _get_provider_instance(
        ...
        Examples:
            >>> from private_gpt.Provider.TTI.dalle import DALLE
            >>> images = client.images
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        Examples:
            ...
            >>> from private_gpt.Provider.TTI.stable import StableDiffusion
            >>> response = client.images.generate(
...
class Client:
    """
    Unified Private_GPT Client for AI chat and image generation.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """
        Initialize the Private_GPT Client with optional configuration.
...
try:
    from private_gpt.server.server import run_api as _run_api_impl
    from private_gpt.server.server import run_api as _start_server_impl

    def run_api(*args: Any, **kwargs: Any) -> Any:
        """
        Runs the FastAPI OpenAI-compatible API server.

        Delegates to private_gpt.server.server.run_api to start an OpenAI-compatible
        ...
        Raises:
            ImportError: If private_gpt.server.server is not available.

        Examples:
            >>> from private_gpt.client import run_api
    ...
    def start_server(*args: Any, **kwargs: Any) -> Any:
        ...
        Delegates to private_gpt.server.server.start_server to initialize and run
        an OpenAI-compatible HTTP API server. This is typically the main entry
        point for starting the private_gpt server in production or development.
        ...
        Examples:
            >>> from private_gpt.client import start_server
            >>> start_server()
        """
    ...
    def run_api(*args: Any, **kwargs: Any) -> Any:
        ...
        Install with: pip install private_gpt[api]

        Raises:
            ImportError: Always raised; server not available in current environment.
        """
        raise ImportError("private_gpt.server.server.run_api is not available.")
    ...
    def start_server(*args: Any, **kwargs: Any) -> Any:
        ...
        Raises:
            ImportError: Always raised; server not available in current environment.
        """
        raise ImportError("private_gpt.server.server.start_server is not available.")
