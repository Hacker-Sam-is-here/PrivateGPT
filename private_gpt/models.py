...
from private_gpt.AIbase import Provider, TTSProvider

# Import TTI base class
BaseImages = None
try:
    from private_gpt.Provider.TTI.base import BaseImages as BaseImagesClass
...
class _LLMModels:
    """
    A class for managing LLM provider models in the private_gpt package.
    """
    ...
    def _get_provider_models(self) -> Dict[str, List[str]]:
        ...
        """
        provider_models = {}
        provider_package = importlib.import_module("private_gpt.Provider")

        for _, module_name, _ in pkgutil.iter_modules(provider_package.__path__):
            try:
                module = importlib.import_module(f"private_gpt.Provider.{module_name}")
...
class _TTSModels:
    """
    A class for managing TTS provider voices in the private_gpt package.
    """
    ...
    def _get_tts_voices(self) -> Dict[str, Union[List[str], Dict[str, str]]]:
        ...
        try:
            ...
            tts_package = importlib.import_module("private_gpt.Provider.TTS")

            # Iterate through TTS modules
            for _, module_name, _ in pkgutil.iter_modules(tts_package.__path__):
                try:
                    module = importlib.import_module(f"private_gpt.Provider.TTS.{module_name}")
                    for attr_name in dir(module):
                        attr = getattr(module, attr_name)
                        if (
...
class _TTIModels:
    """
    A class for managing TTI (Text-to-Image) provider models in the private_gpt package.
    """
    ...
    def _get_tti_models(self) -> Dict[str, List[str]]:
        ...
        provider_models = {}
        tti_package = importlib.import_module("private_gpt.Provider.TTI")
