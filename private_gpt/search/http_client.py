...
from ..exceptions import RatelimitE, TimeoutE, Private_GPTE
    ...
    def request(
        ...
        **kwargs: Any,
        ...
        Raises:
            ...
            Private_GPTE: Other request errors.
        """
        try:
            ...
            if resp.status_code == 200:
                ...
                raise Private_GPTE(f"{resp.url} returned {resp.status_code}")

        except Exception as ex:
            if "time" in str(ex).lower() or "timeout" in str(ex).lower():
                raise TimeoutE(f"{url} {type(ex).__name__}: {ex}") from ex
            raise Private_GPTE(f"{url} {type(ex).__name__}: {ex}") from ex
