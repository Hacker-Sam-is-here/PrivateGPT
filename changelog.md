...
- **Blackbox Provider**: Added new Blackbox AI provider in both standard and OpenAI-compatible formats:
  - `private_gpt/Provider/blackbox.py` - Standard provider implementation with conversation support, streaming/non-streaming responses, and LitAgent fingerprinting
  - `private_gpt/Provider/Openai_comp/blackbox.py` - OpenAI-compatible provider following the same pattern as Groq, Cerebras, and other OpenAI-compatible providers
...
- **Server**: Fixed critical startup failure in `private_gpt/server/server.py` when running with default settings (debug=False, workers=1) due to missing `else` clause in `run_api()` function's uvicorn configuration logic. The server now starts correctly in production mode.
...
- **Reasoning Fields in OpenAI Responses**: Added `reasoning_content` and `reasoning` fields to `ChoiceDelta` and `ChatCompletionMessage` classes in `private_gpt/Provider/Openai_comp/utils.py` for OpenAI-compatible provider responses
...
- **curl_cffi Migration**: Complete migration from `requests` to `curl_cffi` library across all providers:
  - Updated all Provider files (`private_gpt/Provider/`) to use `curl_cffi` for HTTP operations
  - Updated search engine files (`private_gpt/search/`) to use curl_cffi
  - Updated core modules (`private_gpt/scout/core/`) and Extra modules
...
### 🔧 Improved
- **Falcon provider**: `private_gpt/Provider/Falcon.py` - Major refactor to align with QwenLM pattern:
...
### ✨ Added
- **K2Think Provider**: `private_gpt/Provider/k2think.py` - New provider for K2 Think AI (https://www.k2think.ai/guest). Features no-auth guest access, streaming/non-streaming support using `sanitize_stream`, conversation history, optimizer support, and OpenAI-compatible API responses. Fully integrated with Private_GPT's provider discovery system.
- **K2Think OpenAI-Compatible Provider**: `private_gpt/Provider/Openai_comp/k2think.py` - OpenAI-compatible implementation of K2Think provider following the same pattern as DeepInfra, Groq, and Sonus providers. Features direct JSON parsing without `sanitize_stream`, proper OpenAI-compatible streaming chunks, and full compatibility with OpenAI API interface.

### 🚮 Removed
 - **X0GPT provider**: Removed the X0GPT provider from the codebase.

### 🚮 Removed
- **ChatSandbox provider**: Completely removed `private_gpt/Provider/ChatSandbox.py` and `private_gpt/Provider/Openai_comp/chatsandbox.py`. The provider has been deprecated and removed from the codebase.

## [2026.02.01] - 2026-02-01

### 🚮 Removed
- **Algion provider**: Completely removed `private_gpt/Provider/Algion.py` and `private_gpt/Provider/Openai_comp/algion.py`. The provider has been deprecated and removed from the codebase.

## [2026.02.02] - 2026-02-02

### 🔧 Improved
 - **AkashGPT provider**: `private_gpt/Provider/Openai_comp/akashgpt.py` & `private_gpt\Provider\akashgpt.py`- Refactored to remove all `api_key` logic. The provider no longer requires or uses an API key for any operation. The constructor and all internal logic have been updated to reflect this, making AkashGPT fully no-auth and easier to use out of the box.
...
### 🏷️ Renamed
- **OPENAI → Openai_comp**: Renamed `private_gpt/Provider/OPENAI` directory to `private_gpt/Provider/Openai_comp` for clarity and naming consistency.

### 🚮 Removed / Moved
- **BraveAI provider moved**: `private_gpt/Provider/AISEARCH/brave_search.py` → `private_gpt/Provider/UNFINISHED/brave_search.py`. The Brave Search AI Chat API currently returns 404 errors; further investigation needed.
- **BraveAI export removed**: `private_gpt/Provider/AISEARCH/__init__.py` - BraveAI removed from active AISEARCH providers.
- **Legacy wrapper removed**: `lol.py` - Legacy compatibility wrapper removed; canonical implementation is now in `private_gpt/Provider/AISEARCH/ayesoul_search.py`.

### ✨ Added
- **AyeSoul AI Search Provider**: `private_gpt/Provider/AISEARCH/ayesoul_search.py` - New provider using AyeSoul's WebSocket endpoint. Supports streaming/non-streaming, image uploads, and LitAgent-based user-agent generation.
...
- **proxy_manager.py**: `private_gpt/Extra/proxy_manager.py` - New ProxyManager utility for managing and rotating proxies with auto-fetching from public proxy lists. Supports HTTP/SOCKS proxies, health checks, and integration with Private_GPT providers.
### 🔧 Improved
- **AyeSoul stream handling**: `private_gpt/Provider/AISEARCH/ayesoul_search.py` - Prefer `stream` key for response text; robust handling of dict/list payloads; serializes structured content to JSON as needed.
- **AyeSoul export**: `private_gpt/Provider/AISEARCH/__init__.py` - Exported `AyeSoul` for unified import/discovery.
- **All AI providers**: Optimized model fetching to be non-blocking and parallel:
  - 14 OpenAI-compatible providers now use background model fetching:
    - `private_gpt/Provider/Openai_comp/deepinfra.py`
    - `private_gpt/Provider/Openai_comp/DeepAI.py`
    - `private_gpt/Provider/Openai_comp/groq.py`
    - `private_gpt/Provider/Openai_comp/openrouter.py`
    - `private_gpt/Provider/Openai_comp/cerebras.py`
    - `private_gpt/Provider/Openai_comp/textpollinations.py`
    - `private_gpt/Provider/Openai_comp/TogetherAI.py`
    - `private_gpt/Provider/Openai_comp/nvidia.py`
    - `private_gpt/Provider/Openai_comp/huggingface.py`
    - `private_gpt/Provider/Openai_comp/algion.py`
    - `private_gpt/Provider/Openai_comp/upstage.py`
    - `private_gpt/Provider/Openai_comp/typliai.py`
    - `private_gpt/Provider/Openai_comp/easemate.py`
    - `private_gpt/Provider/Openai_comp/freeassist.py`
  - 11 Standalone providers updated for direct background fetch:
    - `private_gpt/Provider/Deepinfra.py`
    - `private_gpt/Provider/HuggingFace.py`
    - `private_gpt/Provider/TogetherAI.py`
    - `private_gpt/Provider/TextPollinationsAI.py`
    - `private_gpt/Provider/Sambanova.py`
    - `private_gpt/Provider/OpenRouter.py`
    - `private_gpt/Provider/Openai.py`
    - `private_gpt/Provider/Nvidia.py`
    - `private_gpt/Provider/Groq.py`
    - `private_gpt/Provider/geminiapi.py`
    - `private_gpt/Provider/cerebras.py`
    - `private_gpt/Provider/Algion.py`
...
- **Type checking compliance**: Fixed 33 type errors across provider implementations to pass strict type checking (`uvx ty check .`):
  ...
  - Fixed dictionary subscript errors in 17 provider files:
    - private_gpt/Provider/AISEARCH/ayesoul_search.py
    - private_gpt/Provider/Cohere.py, ExaAI.py, Gemini.py, HeckAI.py
    - private_gpt/Provider/IBM.py, Netwrck.py, Sambanova.py, TextPollinationsAI.py
    - private_gpt/Provider/ai4chat.py, cleeai.py, julius.py, llama3mitril.py, meta.py
    - private_gpt/Provider/Openai_comp/ibm.py, llmchat.py
...
### 🚮 Removed
- **Deprecated providers**: Removed `oivscode.py` and `K2Think.py` from both `private_gpt/Provider/` and `private_gpt/Provider/Openai_comp/` to streamline the codebase.
...
### 🔧 Improved
- **provider**: private_gpt/Provider/Deepinfra.py & private_gpt/Provider/OPENAI/deepinfra.py - Enhanced model filtering:
...
### 🐛 Fixed
- **fix**: private_gpt/server/server.py - Fixed critical private_gpt-server startup failure when running in production mode (debug=False, workers=1). The issue was a missing `else` clause in the `run_api()` function's uvicorn configuration logic, causing the server to never start when using default settings.
...
### 🔧 Improved
- **provider**: private_gpt/Provider/Ayle.py - Reverse engineered ayle.chat to update available models and configs:
...
### 🔧 Improved
- **refactor**: private_gpt/Provider/AISEARCH/iask_search.py - Streamlined IAsk AI search provider:
...
### � Fixed
- **fix**: private_gpt/Provider/OPENAI/sonus.py - Fixed critical non-stream request error "stream mode is not enabled":
...
### 📝 Documentation
- **docs**: private_gpt/client.py - Generated comprehensive PEP 257-compliant docstrings for all 32+ classes and methods:
...
### 🔧 Improved
- **refactor**: private_gpt/Provider/akashgpt.py - Replaced `cloudscraper` with `curl_cffi` for better performance and TLS/SSL handling. Changed from `cloudscraper.create_scraper()` to `curl_cffi.requests.Session()` for HTTP requests.
- **refactor**: private_gpt/Provider/OPENAI/akashgpt.py - Replaced `cloudscraper` with `curl_cffi` for improved HTTP request handling. Updated session initialization to use `curl_cffi.requests.Session()` with proper proxy support in the `__init__` method.
...
### ✨ Added
- **feat**: private_gpt/search/engines/brave/videos.py - New Brave video search engine that parses HTML to extract video results from YouTube and other platforms. Includes support for duration, view count, channel/uploader, published date, and pagination.
- **feat**: private_gpt/search/engines/brave/news.py - New Brave news search engine that parses HTML to extract news articles with title, source, date, body, and thumbnail images. Includes pagination support.
- **feat**: private_gpt/search/engines/brave/suggestions.py - New Brave search suggestions/autocomplete API that returns rich entity suggestions with metadata (name, description, category, image).
- **feat**: private_gpt/search/engines/brave/images.py - New Brave image search engine using JavaScript-based crawling to extract image results with title, URL, dimensions, and source.

### 🔧 Improved
- **refactor**: private_gpt/search/brave_main.py - Implemented images(), news(), videos(), and suggestions() methods in BraveSearch unified interface to support all new Brave search types.
- **refactor**: private_gpt/search/engines/__init__.py - Updated ENGINES dictionary and __all__ exports to register BraveVideos, BraveNews, and BraveSuggestions in their respective categories.
- **refactor**: private_gpt/cli.py - Added specialized print functions for improved CLI UI:
...
- **refactor**: private_gpt/server/routes.py - Updated web search endpoint documentation to mention Brave support for videos and suggestions. Improved suggestions parameter handling with better fallback logic.
- **refactor**: private_gpt/search/brave_main.py - Implemented images() method for Brave image search (previously raised NotImplementedError).

### 📝 Documentation
- **docs**: Updated CLI help text to reflect Brave support for videos, news, images, and suggestions searches.

### ✨ Added
- **feat**: private_gpt/Provider/AISEARCH/BraveAI.py - New BraveAI search provider supporting AI search and deep research (streaming and non-streaming modes). Updated `private_gpt/Provider/AISEARCH/__init__.py` to export the provider and added documentation to `private_gpt/Provider/AISEARCH/README.md`.

## [2026.01.06] - 2026-01-06

### 🚮 Removed
- **remove**: Completely removed `private_gpt/Provider/AISEARCH/genspark_search.py` provider and all references to Genspark from the codebase
- **remove**: Updated `private_gpt/Provider/AISEARCH/__init__.py` to remove Genspark import and export
- **remove**: Updated documentation in `private_gpt/Provider/AISEARCH/README.md` and `Provider.md` to reflect removal of Genspark provider

### 🔧 Maintenance
- **refactor**: private_gpt/Provider/AISEARCH/monica_search.py - Simplified streaming content extraction by switching to an inline lambda for `sanitize_stream`'s `content_extractor` that returns `chunk["text"]` when present. Removed the now-unused `_extract_monica_content` method. Note: this change intentionally stops persisting `session_id` from stream payloads; reintroduce session handling if session persistence is required.
- **fix**: private_gpt/Provider/AISEARCH/webpilotai_search.py - Use `sanitize_stream`'s `content_extractor` for both streaming and non-streaming flows to parse JSON payloads and return the nested `content` (fallbacks to `text` and `data.delta.content` where present). This aligns behavior with `monica_search` and prevents empty or incomplete output when the service emits nested or delta-based content. Raw mode still returns unprocessed chunks.
- **fix**: private_gpt/Provider/AISEARCH/PERPLEXED_search.py - Fixed critical streaming issues and standardized with other providers:
...
- **remove**: Completely removed `private_gpt/Provider/VercelAI.py` provider file and updated related imports and documentation
- **remove**: Removed `private_gpt/Provider/VercelAI.py` provider file and updated related imports and documentation
...
- **refactor**: Made `lxml` dependency optional in pyproject.toml
  ...
  - Updated `private_gpt/search/base.py` to support lxml for HTML parsing
  - Updated `private_gpt/search/engines/duckduckgo/base.py` to use lxml for HTML parsing
...
- **refactor**: Migrated `private_gpt/Provider/AISEARCH/iask_search.py` from `aiohttp` to `curl_cffi` for HTTP requests, while retaining `aiohttp` for WebSocket connections to improve browser fingerprinting and anti-detection capabilities

### 🐛 Fixed
- **fix**: Fixed ModuleNotFoundError for huggingface_hub in private_gpt versions > 2025.12.19 by implementing lazy imports in gguf.py
...
### ✨ Added
- **remove**: Removed HadadXYZ providers: `private_gpt/Provider/HadadXYZ.py` and `private_gpt/Provider/OPENAI/hadadxyz.py`. These implementations are deprecated and removed to streamline provider maintenance.

- **feat**: private_gpt/Provider/QwenLM.py - Added new models to this provider

- **feat**: Added `gpt-5.2` model to Toolbaz providers in both `private_gpt/Provider/toolbaz.py` and `private_gpt/Provider/OPENAI/toolbaz.py`

- **refactor**: Migrated `private_gpt/Provider/OPENAI/toolbaz.py` from `cloudscraper` to `curl_cffi` for consistent HTTP client implementation across providers

- **refactor**: Migrated `private_gpt/Provider/OPENAI/sonus.py` from `requests` to `curl_cffi` for improved browser emulation and consistency; updated form data handling from multipart `files` to standard `data` parameter

- **feat**: Added new OpenRouter provider in both `private_gpt/Provider/OpenRouter.py` and `private_gpt/Provider/OPENAI/openrouter.py` with dynamic model fetching and LitAgent browser fingerprinting
...
- **docs**: litprinter.md - Completely rewrote documentation to be comprehensive and consistent with other Private_GPT docs. Added detailed sections for IceCream debugging, Rich Console, Panels & Layouts, Traceback Enhancement, Themes & Styling, Advanced Usage, Integration with Private_GPT, API Reference, Dependencies, and Supported Python Versions. Enhanced with professional formatting, extensive code examples, and parameter tables.

### ✨ Added

- **feat**: private_gpt/Provider/Nvidia.py - New standalone Nvidia NIM provider with dynamic model fetching and advanced stream sanitization.
- **feat**: private_gpt/Provider/OPENAI/nvidia.py - New OpenAI-compatible Nvidia NIM provider with manual stream parsing.
- **feat**: private_gpt/Provider/HuggingFace.py - New standalone Hugging Face provider with dynamic model fetching and advanced stream sanitization.
- **feat**: private_gpt/Provider/OPENAI/huggingface.py - New OpenAI-compatible Hugging Face provider using HF Router with manual stream parsing.
- **feat**: private_gpt/Provider/ChatSandbox.py - Major update to ChatSandbox provider after reverse engineering. Expanded `AVAILABLE_MODELS` to include `deepseek-r1-full`, `gemini-thinking`, `llama`, and others.
- **feat**: private_gpt/Provider/OPENAI/chatsandbox.py - Updated OpenAI-compatible ChatSandbox provider with new model list and improved response extraction for reasoning and content.
...
- **feat**: private_gpt/Provider/TTI/together.py - Refactored TogetherImage provider to require a user-provided API key, removing the brittle auto-authentication logic. This aligns it with the TogetherAI chat providers.
...
- **feat**: private_gpt/Provider/TTI/pollinations.py - Expanded supported models list to include `flux-pro`, `flux-realism`, `flux-anime`, and `any-dark`.
- **feat**: `private_gpt/update_checker.py` - Major overhaul of the update system:
...
### 🛠️ Improved
- **refactor**: private_gpt/Provider/TTI/ - Cleaned up the TTI module by removing 8 non-functional or login-required providers (`AIArta`, `BingImageAI`, `GPT1Image`, `ImagenAI`, `InfipAI`, `MonoChatAI`, `PiclumenAI`, `PixelMuse`).
- **refactor**: private_gpt/client.py - Updated unified client to support authenticated TTI providers in auto-failover mode when an API key is provided.
- **docs**: private_gpt/Provider/TTI/README.md - Updated documentation to reflect the current set of 5 functional TTI providers.
- **refactor**: `private_gpt/Provider/AISEARCH/` - Standardized `raw` parameter handling across all providers (Perplexity, Genspark, IAsk, Monica, PERPLEXED, webpilotai). Raw mode now returns/yields unprocessed strings/lines.
- **refactor**: `private_gpt/Provider/AISEARCH/` - Replaced manual SSE parsing with `sanitize_stream` using `extract_regexes` for robust JSON payload extraction in Perplexity, Genspark, and webpilotai.
- **refactor**: `private_gpt/Provider/AISEARCH/` - Standardized internal state tracking by renaming `last_SearchResponse` to `last_response` across all providers for better consistency.
- **fix**: `private_gpt/Provider/AISEARCH/webpilotai_search.py` - Fixed critical issue where search results were empty due to incorrect content accumulation logic; now correctly handles delta-based streaming.
- **fix**: `private_gpt/Provider/AISEARCH/iask_search.py` - Improved raw mode output to yield raw strings instead of dictionaries for better consistency with other providers.
- **refactor**: `private_gpt/Provider/AISEARCH/` - Enhanced `sanitize_stream` configuration with explicit encoding, buffer handling, and custom `content_extractor` functions to maintain provider-specific metadata (sources, status updates) during streaming.

### 🛠️ Improved
- **docs**: docs/cli.md - Updated CLI documentation to reflect all available commands, options, and examples based on actual implementation in private_gpt/cli.py.
...
### ✨ Added
- **feat**: private_gpt/Provider/TTS/sherpa.py - New SherpaTTS provider using Next-gen Kaldi (Sherpa-ONNX) HF Space API, supporting 50+ languages and multiple ONNX models.
- **feat**: private_gpt/Provider/TTS/qwen.py - New Qwen3-TTS provider reverse engineered from Hugging Face Space demo, supporting 40+ high-quality voices and automatic language detection.
- **feat**: private_gpt/Provider/TTS/deepgram.py - Updated to support Aura-2 next-gen voices and latest API endpoint.
- **feat**: private_gpt/Provider/TTS/elevenlabs.py - Added support for ElevenLabs API keys.
- **feat**: private_gpt/Provider/TTI/miragic.py - New MiragicAI TTI provider reverse engineered from Hugging Face Space, supporting 'flux', 'turbo', and 'gptimage' models with streaming support.
- **feat**: private_gpt/server/routes.py - Added `/monitor/health` endpoint for Docker health checks, returning service status and version information.

### 🛠️ Improved
- **fix**: private_gpt/Provider/TTS/freetts.py - Fixed 404 error by updating to the latest synthesis API and polling mechanism.
- **fix**: private_gpt/Provider/TTS/parler.py - Switched to manual Gradio polling for improved reliability and timeout handling.
- **removed**: private_gpt/Provider/TTS/gesserit.py - Removed dead TikTok TTS provider.
- **refactor**: private_gpt/Provider/TTS/__init__.py - Added QwenTTS and SherpaTTS to the exported TTS providers.
- **refactor**: private_gpt/Provider/OPENAI/__init__.py - Added LLMChat to the list of exported OpenAI-compatible providers.
- **refactor**: private_gpt/Provider/OPENAI/e2b.py - Removed all unwanted `print` statements and ANSI escape codes to make the provider fully silent.
- **refactor**: private_gpt/Provider/TTS/ - Updated all TTS providers to use `ic` from litprinter instead of `print` for debugging, following the pattern established in deepgram.py:
...
- **removed**: `gemini-2.0-flash` from Ayle provider model lists (`private_gpt/Provider/Ayle.py`, `private_gpt/Provider/OPENAI/ayle.py`).
- **removed**: private_gpt/Provider/AISEARCH/stellar_search.py - Removed dead Stellar AI search provider that was causing import errors and service unavailability.

### 🐛 Fixed
- **fix**: private_gpt/Provider/meta.py - Fixed HTTP/2 stream closure errors (curl error 92) by implementing robust retry mechanism with exponential backoff using AIutel retry decorator
- **fix**: private_gpt/Provider/meta.py - Added HTTP/1.1 fallback via requests library when HTTP/2 streams fail, ensuring compatibility with problematic networks
- **fix**: private_gpt/Provider/meta.py - Improved cookie extraction with retry logic, fallback to browser cookies, and better error handling for region-blocked sites
- **fix**: private_gpt/Provider/meta.py - Added `skip_init` parameter for offline testing and development without requiring live cookie fetching
- **fix**: private_gpt/Provider/meta.py - Enhanced streaming error recovery to gracefully handle connection interruptions during response iteration
- **fix**: private_gpt/Provider/meta.py - Improved main module error handling with helpful guidance for network issues and testing modes
- **test**: Added comprehensive unit tests in `tests/test_meta_http2.py` for HTTP/2 fallback functionality and retry mechanisms
- **fix**: private_gpt/Provider/oivscode.py - Fixed type annotation bugs by updating optional parameters to use `Optional[str]` type hints, correcting method calls, and adding proper string handling in `get_message()` method
- **fix**: private_gpt/Provider/oivscode.py - Removed unwanted print statements from `fetch_available_models()` method and updated its docstring to reflect that it no longer prints models
- **fix**: private_gpt/sanitize.py - Fixed import organization issues using `ruff check --fix` to properly sort and format imports according to project standards
- **fix**: private_gpt/scout - Fixed multiple critical bugs, removed direct BeautifulSoup references and updated BS4-compatible messaging across the scout component. (Applied ruff checks and whitespace fixes)
- **fix**: private_gpt/scout/core/scout.py - Resolved `TypeError` in `extract_metadata` and standardized `find` return types.
- **fix**: private_gpt/scout/core/crawler.py - Improved domain validation security, fixed overly aggressive URL normalization, and corrected parser selection logic.
- **fix**: private_gpt/scout/element.py - Fixed class matching logic and self-closing tag rendering in `prettify`.
- **fix**: private_gpt/scout/parsers/lxml_parser.py - Implemented XML namespace stripping for cleaner tag names.
- **fix**: private_gpt/Provider/QwenLM.py - Fixed multiple critical bugs including syntax errors, indentation issues, and formatting problems. Applied ruff check fixes to resolve blank lines with whitespace, trailing whitespace, and proper docstring formatting.

## [2025.12.19] - 2025-12-19

### ✨ Added
  - **feat**: private_gpt/client.py - Enhanced unified Client interface for AI providers:
  ...
  - **feat**: private_gpt/Provider/OPENAI/typliai.py - New OpenAI-compatible TypliAI provider with streaming and non-streaming support for GPT-4.1, GPT-5, Gemini 2.5, Claude 4.5, and Grok 4 models.
  - **feat**: private_gpt/Provider/OPENAI/easemate.py - New OpenAI-compatible Easemate provider.
  - **feat**: private_gpt/Provider/OPENAI/freeassist.py - New OpenAI-compatible FreeAssist provider.
  
  ### 🔧 Improved
  - **refactor**: private_gpt/client.py - Robust model discovery logic that safely handles class attributes, instance properties (including `@property`), and `.models.list()` methods during model resolution and fuzzy matching.
  - **refactor**: private_gpt/Provider/Sambanova.py - Major refactor to align with TextPollinationsAI pattern:  - Implemented `sanitize_stream` for robust SSE parsing.
...
### 🚮 Removed
- **removed**: private_gpt/Provider/Perplexitylabs.py - Removed PerplexityLabs provider file.
- **removed**: PerplexityLabs entry from Provider.md documentation and statistics.
- **removed**: References to PerplexityLabs from private_gpt/Provider/__init__.py.
- **removed**: private_gpt/Provider/TeachAnything.py - Removed TeachAnything provider file.
- **removed**: TeachAnything entry from Provider.md documentation and statistics.
- **removed**: References to TeachAnything from private_gpt/Provider/__init__.py.
- **removed**: private_gpt/Provider/GeminiProxy.py - Removed GeminiProxy provider file.
- **removed**: GeminiProxy entry from Provider.md documentation and statistics.
- **removed**: References to GeminiProxy from private_gpt/Provider/__init__.py.

### 🛠️ Fixed
- **fix**: private_gpt/Provider/OPENAI/ - Fixed "Accept-Encoding" issue in multiple providers (`K2Think`, `AkashGPT`, `LLMChatCo`, `Yep`, `Zenmux`, `DeepInfra`) that caused decompression errors and empty responses when using `requests` or `cloudscraper` libraries.
- **fix**: private_gpt/Provider/turboseek.py - Updated provider to handle new HTML-based raw stream response format and improved HTML-to-Markdown conversion.
...
### 🚮 Removed
- **removed**: private_gpt/Provider/Nemotron.py - Removed Nemotron provider as the file doesn't exist and was causing import errors
- **removed**: References to NEMOTRON from private_gpt/Provider/__init__.py
- **removed**: Nemotron entry from Provider.md documentation

- **feat**: private_gpt/Provider/OPENAI/gradient.py - New OpenAI-compatible Gradient Network provider for accessing distributed GPU clusters with models GPT OSS 120B and Qwen3 235B
- **feat**: private_gpt/Provider/OPENAI/gradient.py - Supports both streaming and non-streaming modes with thinking/reasoning capability
- **feat**: private_gpt/Provider/OPENAI/gradient.py - Auto-detection of cluster mode per model (nvidia for GPT OSS 120B, hybrid for Qwen3 235B)
- **feat**: private_gpt/Provider/freeassist.py - New OpenAI-compatible FreeAssist provider using FreeAssist.ai API with access to multiple AI models including gemini 2.5 flash and flash lite and GPT-5-nano and GPT-5-mini
- **feat**: private_gpt/Provider/OPENAI/sambanova.py - New OpenAI-compatible Sambanova provider supporting Llama 3.1/3.3, Qwen, and DeepSeek models with streaming capabilities
- **feat**: private_gpt/Provider/OPENAI/meta.py - New OpenAI-compatible Meta AI provider with web authentication, optional Facebook login, and streaming support

### 🔧 Improved

- **refactor**: private_gpt/Provider/Gradient.py - Major rewrite with correct headers matching actual API, proper SSE response parsing for content/reasoningContent
- **refactor**: private_gpt/Provider/Gradient.py - Now uses sanitize_stream with custom _gradient_extractor following the pattern of other providers
- **refactor**: private_gpt/Provider/Gradient.py - Added MODEL_CLUSTERS mapping for auto-detection of cluster mode (nvidia for GPT, hybrid for Qwen3)
- **refactor**: private_gpt/Provider/Gradient.py - Updated model names to use spaces (GPT OSS 120B, Qwen3 235B) matching API format
- **feat**: private_gpt/Provider/OPENAI/freeassist.py - Supports both streaming and non-streaming modes with proper SSE parsing
- **feat**: private_gpt/Provider/OPENAI/zenmux.py - Implemented dynamic model list fetching from `https://zenmux.ai/api/v1/models` API endpoint, making it fully compatible with Groq provider pattern
- **refactor**: private_gpt/Provider/TextPollinationsAI.py - Switched to requests library and implemented proper non-streaming support via `stream=False` to match API behavior
- **fix**: private_gpt/Provider/OPENAI/textpollinations.py - Fixed duplicate code blocks and syntax errors, ensuring proper class structure and dynamic model fetching

### 🚮 Removed
- **removed**: private_gpt/Provider/OPENAI/FreeGemini.py - Removed FreeGemini provider due to service deprecation
- **removed**: private_gpt/Provider/OpenGPT.py - Removed OpenGPT provider from the project

### 🔧 Maintenance
- **refactor**: private_gpt/Provider/OPENAI/DeepAI.py - Implemented dynamic model fetching using `get_models()` and `update_available_models()` class methods following Cerebras provider pattern
- **refactor**: private_gpt/Provider/OPENAI/textpollinations.py - Implemented dynamic model fetching using `get_models()` and `update_available_models()` class methods following Cerebras provider pattern
- **refactor**: private_gpt/Provider/TTI/together.py - Implemented dynamic model fetching using `get_models()` and `update_available_models()` class methods following Cerebras provider pattern
...
- **feat**: private_gpt/Provider/OPENAI/zenmux.py - Added `get_models()` and `update_available_models()` class methods for automatic model discovery and updating AVAILABLE_MODELS on initialization

#### GGUF Converter v2.0 Major Update
- **feat**: private_gpt/Extra/gguf.py - Upgraded to version 2.0.0 with latest llama.cpp features
...
- **feat**: private_gpt/Extra/GitToolkit/gitapi/search.py - New `GitSearch` class with methods for GitHub Search API: `search_repositories()`, `search_users()`, `search_topics()`, `search_commits()`, `search_issues()`, `search_labels()`
- **feat**: private_gpt/Extra/GitToolkit/gitapi/gist.py - New `Gist` class for public gist operations: `get()`, `list_public()`, `list_for_user()`, `get_commits()`, `get_forks()`, `get_revision()`, `get_comments()`
- **feat**: private_gpt/Extra/GitToolkit/gitapi/organization.py - New `Organization` class for org data: `get_info()`, `get_repos()`, `get_public_members()`, `get_events()`
- **feat**: private_gpt/Extra/GitToolkit/gitapi/trending.py - New `Trending` class for GitHub trending: `get_repositories()`, `get_developers()`
- **feat**: private_gpt/Extra/GitToolkit/gitapi/repository.py - Added 9 new methods: `get_readme()`, `get_license()`, `get_topics()`, `get_forks()`, `get_stargazers()`, `get_watchers()`, `compare()`, `get_events()`
- **feat**: private_gpt/Extra/GitToolkit/gitapi/user.py - Added 2 new methods: `get_social_accounts()`, `get_packages()`

#### YTToolkit Enhancements
- **feat**: private_gpt/Extra/YTToolkit/ytapi/suggestions.py - New `Suggestions` class for YouTube autocomplete: `autocomplete()`, `trending_searches()`
- **feat**: private_gpt/Extra/YTToolkit/ytapi/shorts.py - New `Shorts` class for YouTube Shorts: `is_short()`, `get_trending()`, `search()`
- **feat**: private_gpt/Extra/YTToolkit/ytapi/hashtag.py - New `Hashtag` class for hashtag videos: `get_videos()`, `get_metadata()`, `extract_from_text()`
- **feat**: private_gpt/Extra/YTToolkit/ytapi/captions.py - New `Captions` class for video transcripts: `get_available_languages()`, `get_transcript()`, `get_timed_transcript()`, `search_transcript()`
- **feat**: private_gpt/Extra/YTToolkit/ytapi/video.py - Added new properties/methods: `is_live`, `is_short`, `hashtags`, `get_related_videos()`, `get_chapters()`, `stream_comments()`
- **feat**: private_gpt/Extra/YTToolkit/ytapi/query.py - Added new search methods: `shorts()`, `live_streams()`, `videos_by_duration()`, `videos_by_upload_date()`
- **feat**: private_gpt/Extra/YTToolkit/ytapi/extras.py - Added new trending methods: `shorts_videos()`, `movies()`, `podcasts()`

### 🔧 Improved
- **refactor**: private_gpt/Extra/YTToolkit/transcriber.py - Rewrote YTTranscriber to use YouTube's InnerTube API for more reliable transcript fetching, replacing brittle HTML parsing with direct API calls
...
### 🔧 Maintenance
- **refactor**: private_gpt/Extra/GitToolkit/gitapi/__init__.py - Updated exports to include new classes
- **refactor**: private_gpt/Extra/YTToolkit/ytapi/__init__.py - Updated exports to include new classes
- **docs**: private_gpt/Extra/GitToolkit/gitapi/README.md - Updated documentation with all new features and examples
- **docs**: private_gpt/Extra/YTToolkit/README.md - Updated documentation with all new features and examples
- **refactor**: private_gpt/Extra/gguf.py - Updated conversion logic to use dynamic outtype instead of hardcoded f16
- **refactor**: private_gpt/Extra/gguf.py - Improved split size validation to support K, M, G units matching llama.cpp
- **refactor**: private_gpt/Extra/gguf.py - Added outtype validation against VALID_OUTTYPES set
- **docs**: Moved private_gpt/Extra/gguf.md → docs/gguf.md for better documentation organization
- **docs**: docs/gguf.md - Complete rewrite for v2.0 with all new features, examples, and troubleshooting

### 🚮 Removed
- **removed**: private_gpt/Extra/autocoder/ - Completely removed AutoCoder package directory and all its files.
- **refactor**: private_gpt/AIutel.py - Removed AutoCoder import.
- **refactor**: private_gpt/Extra/__init__.py - Removed AutoCoder import.
...
- **feat**: private_gpt/Provider/OPENAI/utils.py - Added dict-like access methods (`__getitem__`, `__setitem__`, `keys`, `values`, `items`) to `ChatCompletionMessage` class for better compatibility
- **feat**: private_gpt/Provider/OPENAI/PI.py - Added missing `count_tokens` import for proper token counting functionality

### 🐛 Fixed
- **fix**: private_gpt/server/providers.py - Added `required_auth = False` filtering to only initialize OpenAI-compatible providers that don't require authentication, improving server startup and reducing provider count to 28 no-auth providers

### 🔧 Maintenance
- **refactor**: Replaced Litlogger with litprinter across entire codebase for consistent logging:
  - **refactor**: private_gpt/Extra/autocoder/autocoder.py - Updated logger initialization comment
  - **refactor**: private_gpt/Extra/tempmail/async_utils.py - Replaced standard logging with litprinter
  - **refactor**: private_gpt/Provider/OPENAI/K2Think.py - Replaced Litlogger imports with litprinter
  - **refactor**: private_gpt/Provider/OPENAI/base.py - Replaced Litlogger with litprinter for error logging
  - **refactor**: private_gpt/Provider/TTS/speechma.py - Replaced Litlogger with litprinter
  - **refactor**: private_gpt/Provider/meta.py - Removed unused logging import
  - **refactor**: private_gpt/__init__.py - Removed Litlogger import
  - **refactor**: private_gpt/conversation.py - Replaced logging with litprinter
  - **refactor**: private_gpt/search/base.py - Replaced logging with litprinter
  - **refactor**: private_gpt/search/engines/wikipedia.py - Replaced logging with litprinter
  - **refactor**: private_gpt/search/http_client.py - Replaced logging with litprinter
  - **refactor**: private_gpt/server/config.py - Replaced Litlogger with litprinter
  - **refactor**: private_gpt/server/providers.py - Replaced Litlogger with litprinter
  - **refactor**: private_gpt/server/request_processing.py - Replaced Litlogger with litprinter and added inline utility functions
  - **refactor**: private_gpt/server/routes.py - Replaced Litlogger with litprinter
  - **refactor**: private_gpt/server/server.py - Replaced Litlogger with litprinter
- **refactor**: private_gpt/search/engines/__init__.py - Changed from auto-discovery to static imports for better performance and reliability
- **refactor**: private_gpt/Provider/AISEARCH/__init__.py - Cleaned up import comments
- **refactor**: private_gpt/server/request_processing.py - Added inline implementations of `get_client_ip()`, `generate_request_id()`, and `log_api_request()` functions to replace dependency on simple_logger.py
...
- **removed**: private_gpt/Litlogger/ - Completely removed Litlogger package directory and all its files (README.md, __init__.py, formats.py, handlers.py, levels.py, logger.py)
- **removed**: private_gpt/litprinter/__init__.py - Removed redundant wrapper file
- **removed**: private_gpt/server/simple_logger.py - Deleted file as functionality moved inline to request_processing.py

## [2025.12.03] - 2025-12-03

### ✨ Added
- **feat**: private_gpt/search/engines/__init__.py - Updated auto-discovery logic to register all search engine classes with `name` and `category` attributes, not just BaseSearchEngine subclasses
- **feat**: private_gpt/server/routes.py - Added new `/search/provider` endpoint that returns details about each search provider including name, supported categories, and parameters
- **feat**: private_gpt/models.py - Enhanced LLM models class with `providers()` and `provider()` methods that return detailed provider information including models, parameters, and metadata
- **feat**: private_gpt/models.py - Added TTI (Text-to-Image) models support with `_TTIModels` class including detailed provider information methods
...
### 🐛 Fixed
- **fix**: private_gpt/server/routes.py - Fixed search engine method checking bug where it was looking for `hasattr(searcher, type)` instead of `hasattr(searcher, "run")`, preventing DuckDuckGo and other engines from working
- **fix**: private_gpt/server/routes.py - Fixed FastAPI UI documentation issue where search engines were listed multiple times by using `set()` to get unique engine names
- **fix**: private_gpt/search/engines/brave.py - Added `run` method to Brave search engine class for compatibility with search endpoint
- **fix**: private_gpt/search/engines/mojeek.py - Added `run` method to Mojeek search engine class for compatibility with search endpoint
- **fix**: private_gpt/search/engines/yandex.py - Added `run` method to Yandex search engine class for compatibility with search endpoint
- **fix**: private_gpt/search/engines/wikipedia.py - Added `run` method to Wikipedia search engine class for compatibility with search endpoint
- **fix**: private_gpt/models.py - Fixed `_LLMModels.summary()` method which was missing its return statement, causing it to return `None` instead of the expected dictionary with provider and model counts
...
- **refactor**: Updated private_gpt/search/engines/bing/__init__.py to import and expose Bing search engine classes
...
- **removed**: private_gpt/server/routes.py - Removed monitoring endpoints (`/monitor/requests`, `/monitor/stats`, `/monitor/health`) and related code from server
- **removed**: private_gpt/server/simple_logger.py - Removed unused monitoring methods (`get_recent_requests`, `get_stats`) from SimpleRequestLogger class
...
### ✨ Added
 ...
 - **feat**: OPENAI/ibm.py - Added OpenAI-compatible IBM Granite provider in `private_gpt/Provider/OPENAI/` with support for `granite-chat` and `granite-search` models
...
### 🔧 Maintenance
 - **refactor**: AIauto.py - Improved robustness of AUTO provider to work seamlessly with all providers in private_gpt.Provider package
...
### 🐛 Fixed
 - **fix**: ChatGPT provider - Fixed OpenAI compatibility issues in `private_gpt/Provider/OPENAI/chatgpt.py` by updating streaming and non-streaming implementations to properly handle Server-Sent Events format and match OpenAI's response structure exactly
 - **fix**: ChatGPT provider - Enhanced error handling and parameter validation to follow OpenAI conventions
 - **fix**: AkashGPT provider - Fixed authentication issue in `private_gpt/Provider/akashgpt.py` by updating API key handling to use cookies for authentication
...
- **feat**: added `moonshotai/Kimi-K2-Thinking` and `MiniMaxAI/MiniMax-M2` models to DeepInfra provider AVAILABLE_MODELS in both `private_gpt/Provider/Deepinfra.py` and `private_gpt/Provider/OPENAI/deepinfra.py`
- **feat**: 
...
### ✨ Added
- **feat**: Initial release with core Private_GPT functionality (1a2b3c4) — Added web scraping, AI provider integration, and base CLI tooling.
...
For more details, see the [documentation](docs/) or [GitHub repository](https://github.com/pyscout/Private_GPT).
---
