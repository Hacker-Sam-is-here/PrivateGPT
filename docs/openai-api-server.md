# OpenAI-Compatible API Server (`private_gpt.server`)
> Last updated: 2025-12-20
> Maintained by [Private_GPT](https://github.com/OEvortex/Private_GPT)

Private_GPT's [`private_gpt.server`](../private_gpt/server/__init__.py:1) module provides a comprehensive OpenAI-compatible API server that serves AI models in OpenAI-compatible API format, making it usable wherever OpenAI API is expected. This server allows you to use any supported Private_GPT provider with tools and applications designed for OpenAI's API. The server exposes the same providers available in the Private_GPT Python client (`client.py`) through HTTP endpoints, enabling integration with any OpenAI-compatible application. For client-side integrations, see [docs/client.md](client.md).
...
### [`server.py`](../private_gpt/server/server.py:1)

The main server module that creates and configures the FastAPI application with OpenAI-compatible endpoints.

```python
from private_gpt.server.server import create_app, run_api, start_server
...
### [`ServerConfig`](../private_gpt/server/config.py:22)

Centralized configuration management for the API server.

```python
from private_gpt.server.config import ServerConfig
...
### [`providers.py`](../private_gpt/server/providers.py:1)
...
```python
from private_gpt.server.providers import (
...
**Key Features:**
- Discovers providers automatically at startup from `private_gpt.Provider.OPENAI`
...
private_gpt-server --port 8080 --host 127.0.0.1 --debug

# Production settings
private_gpt-server --port 8000 --host 0.0.0.0 --workers 4 --log-level info
```
...
```python
from private_gpt.server import start_server, run_api
...
python -m private_gpt.server.server

# Using Python module with arguments
python -m private_gpt.server.server --port 8080 --debug

# Direct execution
python private_gpt/server/server.py --host localhost --port 9000
```
...
### [`APIError`](../private_gpt/server/exceptions.py:26)

Comprehensive error handling with OpenAI-compatible error responses.

```python
from private_gpt.server.exceptions import APIError
...
```json
{
  "error": {
    ...
    "param": "model",
    "code": "model_not_found",
    "footer": "If you believe this is a bug, please file an issue at https://github.com/OEvortex/Private_GPT."
  }
}
```
...
### Landing Page
The root endpoint (`/`) serves a custom landing page with information about the Private_GPT API server and available features.

### API Documentation
- `/docs`: Custom Swagger UI with Private_GPT branding and GitHub footer
...
2. **Model names**: Use Private_GPT provider/model format (e.g., "ChatGPT/gpt-4o", "Toolbaz/grok-4.1-fast")
...
```bash
private_gpt-server --log-level debug
```
...
docker-compose logs private_gpt-api
```

*This documentation covers the comprehensive functionality of the [`private_gpt.server`](../private_gpt/server/__init__.py:1) module. For the most up-to-date information, refer to the source code and inline documentation.*
