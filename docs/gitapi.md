...
Private_GPT's GitAPI module provides a powerful, lightweight GitHub data extraction toolkit within the Private_GPT Python package. It offers comprehensive tools for retrieving GitHub repository, user, organization, gist, and trending data without requiring authentication for public data access.
...
### [`gitapi.py`](../private_gpt/Extra/GitToolkit/gitapi/__init__.py:1)

The main GitHub API module that provides comprehensive GitHub data extraction capabilities.

```python
from private_gpt.Extra.GitToolkit.gitapi import Repository, User, GitSearch, Gist, Organization, Trending

# Initialize and use GitHub data extraction
repo = Repository("OE-LUCIFER", "Private_GPT")
info = repo.get_info()
...
Install as part of the Private_GPT package:
...
```python
from private_gpt.Extra.GitToolkit.gitapi import Repository
...
```python
from private_gpt.Extra.GitToolkit.gitapi import User
...
```python
from private_gpt.Extra.GitToolkit.gitapi import GitSearch

search = GitSearch()

# Search repositories
repos = search.search_repositories("private_gpt language:python stars:>100")
...
```python
from private_gpt.Extra.GitToolkit.gitapi import Gist
...
```python
from private_gpt.Extra.GitToolkit.gitapi import Organization
...
```python
from private_gpt.Extra.GitToolkit.gitapi import Trending
...
```python
from private_gpt.Extra.GitToolkit.gitapi import Repository, NotFoundError, RateLimitError
...
from private_gpt.Extra.GitToolkit.gitapi import Repository, NotFoundError
...
```python
import pandas as pd
from private_gpt.Extra.GitToolkit.gitapi import Repository, GitSearch
...
search = GitSearch()
results = search.search_repositories("private_gpt language:python")
...
print(f"\nPrivate_GPT-related repositories:")
...
```python
import click
from private_gpt.Extra.GitToolkit.gitapi import Repository, User, GitSearch
...
import time
from private_gpt.Extra.GitToolkit.gitapi import RateLimitError
...
*This documentation covers the comprehensive functionality of the [`private_gpt.Extra.GitToolkit.gitapi`](../private_gpt/Extra/GitToolkit/gitapi/__init__.py:1) module. For the most up-to-date information, refer to the source code and inline documentation.*
