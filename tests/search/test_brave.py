...
from private_gpt.search.engines.brave.base import BraveBase
from private_gpt.search.engines.brave.text import BraveTextSearch
...
class TestBraveBase(unittest.TestCase):
  def test_brave_base_uses_curl_cffi_session(self) -> None:
    ...
    with patch("private_gpt.search.engines.brave.base.Session", return_value=session_mock) as mock_session:
      with patch("private_gpt.search.engines.brave.base.LitAgent") as mock_litagent:
...
class TestBraveTextSearch(unittest.TestCase):
    def test_run_parses_results_from_brave_html(self) -> None:
        ...
        with patch.object(searcher.session, "get", return_value=FakeResp(text=html)) as mock_get:
            results = searcher.run("private_gpt", max_results=1)
        ...
        self.assertEqual(call_kwargs["params"]["q"], "private_gpt")
